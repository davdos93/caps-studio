...
const html=fs.readFileSync("beta/index.html","utf8"),scripts=[...html.matchAll(/<script src="([^"?]+)/g)].map(m=>m[1]);
for(const ref of scripts)vm.runInContext(fs.readFileSync("beta/"+ref,"utf8"),sandbox,{filename:ref});
if(!appNode.innerHTML.includes("Was soll in einem Kinderherzen bleiben?"))throw new Error("Home fehlt");
if(sandbox.CAPS.version!=="0.7.12")throw new Error("Version falsch");
if(!sandbox.CAPS_StoryTreatmentEngine)throw new Error("Treatment Engine fehlt");
console.log(JSON.stringify({scripts:scripts.length,homeRendered:true,version:sandbox.CAPS.version,treatmentEngine:true},null,2));
